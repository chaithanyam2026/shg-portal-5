  ...
  async function save() {
    try {
        ...
        `/api/financial-years/${financialYear._id}`,
        {
          ...
          body: JSON.stringify({
            executiveCommittee: {
              ...
              treasurer:
                committee.treasurer || null,
            },
            members: financialYear.members.map(
              (member) => ({
                memberId: member.memberId._id,

                openingContribution:
                  member.opening.contribution ?? 0,

                openingLoan:
                  member.openingLoan ?? 0,

                openingSpecialLoan:
                  member.openingSpecialLoan ?? 0,

                specialLoanExpiry:
                  member.specialLoanExpiry ?? null,
              }),
            ),
          }),
        },
      ...
      if (err instanceof Error) {
        ...
        setError(
          "Unable to save executive committee.",
        );
      }
    } finally {
      setSaving(false);
