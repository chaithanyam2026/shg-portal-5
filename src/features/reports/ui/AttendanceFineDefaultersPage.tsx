...
import AttendanceFineDefaultersTable from "./AttendanceFineDefaultersTable";
...
export default function AttendanceFineDefaultersPage({ financialYearId, options, report }: Props) {
  ...
  function handleChange(financialYearId: string) {
    ...
    router.push(`/reports/attendance-fine-defaulters?${params.toString()}`);
  }
  ...
  return (
    <Stack spacing={3}>
      ...
      <AttendanceFineDefaultersTable report={report} />
    </Stack>
  );
}
